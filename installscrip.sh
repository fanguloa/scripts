...
apt-get install node-less -y
 ...
 apt-get install libssl-dev libxml2-dev libxmlsec1-dev build-essential libssl-dev libffi-dev python3-dev python-suds swig python-dev python-cffi libxml2-dev libxslt1-dev libssl-dev python-lxml python-cryptography python-openssl python-certifi python-defusedxml python3-pip wget -y
...
sleep 3s
 git clone --branch 11.0 https://gitlab.com/dansanti/l10n_cl_dte_point_of_sale.git
 git clone --branch 11.0 https://gitlab.com/dansanti/l10n_cl_fe.git
 git clone --branch 11.0 https://gitlab.com/dansanti/payment_khipu.git
 git clone --branch 11.0 https://gitlab.com/dansanti/payment_webpay.git
# git clone https://gitlab.com/dansanti/l10n_cl_stock_picking.git
 git clone --branch 11.0 https://github.com/OCA/reporting-engine.git 
 git clone --branch 11.0 https://github.com/KonosCL/addons-konos.git
 mv addons-konos/l10n_cl_chart_of_account/ /odoo/custom/addons/
 rm -rf addons-konos/
...
if [ $IS_ENTERPRISE = "True" ]; then
     ...
     su root -c "printf 'addons_path=${OE_HOME_EXT}/addons,${OE_HOME}/custom/addons,${OE_HOME}/custom/addons/reporting-engine\n' >> /etc/${OE_CONFIG}.conf"
fi
 chown $OE_USER:$OE_USER /etc/${OE_CONFIG}.conf
 chmod 640 /etc/${OE_CONFIG}.conf
echo -e "REINICIANDO SERVICIO ODOO"
sleep 3s
 wget https://raw.githubusercontent.com/tdcomcl/Script_instalar_odoo11_dte/master/requirements.txt
...
echo -e "* Starting Odoo Service"
 su root -c "/etc/init.d/$OE_CONFIG stop"
sleep 5s
